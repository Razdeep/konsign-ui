...
const BillEntry: React.FC = () => {
  ...
  const handleTransportNameChange = (
  ...
  useEffect(() => {
    ...
    fetchedTransportsPromise.then((fetchedTransports) => {
      ...
      const transportNames = fetchedTransports.map(
        (fetchedTransport) => fetchedTransport.transportName,
      )
          ...
          <Grid item lg={4} md={6}>
            <Stack spacing={2}>
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} name="supplierName" size="small" label="Supplier name" />
                )}
              />
              <Autocomplete
                disablePortal
              ...
              <Table sx={{ minWidth: '50vw' }} aria-label="simple table">
                <TableHead>
                  <TableRow sx={{ bgcolor: 'primary.main' }}>
                    <TableCell
                      sx={{ minWidth: 250, color: 'secondary.main' }}
                      variant="head"
                      align="center"
                    >
                      LR
                    ...
                    <TableCell
                      sx={{ minWidth: 150, color: 'secondary.main' }}
                      variant="head"
                      align="center"
                    >
                      Operations
                  ...
                  {lrPmList.map((row, i) => (
                    <TableRow key={i} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      <TableCell align="center">
                        {idxAtEditMode === i ? (
                          <TextField
                            sx={{ width: 80 }}
                            name="lr"
                            value={currentLrPm.lr}
                            onChange={handleLrPmChange}
                            variant="standard"
                            size="small"
                          ></TextField>
                        ) : (
                      ...
                      <TableCell align="center">
                        <ButtonGroup>
                          {idxAtEditMode === i ? (
                            <Button onClick={() => updateLrRow(i)}>
                              <Done></Done>
                            </Button>
                          ) : (
                            <Button onClick={() => startEditingLrPmRow(i)}>
                              <Edit></Edit>
                            </Button>
                          )}
                          <Button onClick={() => deleteRow(i)} color="error">
                            <Delete></Delete>
                          </Button>
                        </ButtonGroup>
                      </TableCell>
